...
def build_messages(sample: dict[str, Any]) -> list[dict[str, Any]]:
    """Build a video-frames + independent-WAV + text conversation."""
...
def run(args: argparse.Namespace) -> None:
    ...
    output_pt = os.path.join(
        args.output_dir, f"{model_name}-CMM-video-driven-hidden-states.pt"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract Qwen2.5-Omni responses and hidden states for CMM video-driven data."
    )
    parser.add_argument(
        "--data_file",
        default="cmm-video-driven.json",
        help="Path to cmm_video_driven.json.",
    )
    parser.add_argument(
        "--model_path",
