...
def run(args: argparse.Namespace) -> None:
    ...
    output_pt = os.path.join(
        args.output_dir, f"{model_name}-CMM-audio-driven-hidden-states.pt"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract Qwen2.5-Omni responses and hidden states for CMM audio-driven data."
    )
    parser.add_argument(
        "--data_file",
        default="cmm-audio-driven.json",
        help="Path to cmm_audio_driven.json.",
    )
    parser.add_argument(
        "--model_path",
