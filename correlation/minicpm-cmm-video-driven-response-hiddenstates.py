...
def run(args: argparse.Namespace) -> None:
    ...
    model_outputs = []
    predictions = []
    for sample in tqdm(samples, desc="CMM video-driven"):
    ...
    json_path = os.path.join(
        args.output_dir, f"{model_name}-CMM-video-driven-results.json"
    )
    hidden_path = os.path.join(
        args.output_dir, f"{model_name}-CMM-video-driven-hiddenstates.pt"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data_file", default="cmm-video-driven.json"
    )
    parser.add_argument(
        "--model_path",
