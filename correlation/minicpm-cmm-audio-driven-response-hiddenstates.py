...
def load_audio(audio_path: str, target_sample_rate: int = 16000) -> np.ndarray:
...
def resolve_media_path(stored_path: str, data_file: str, cmm_root: str | None) -> str:
    ...
    if not stored_path:
        raise ValueError("The sample has an empty media path.")
    ...
    if cmm_root:
        ...
        relative_parts = parts[cmm_indices[-1] + 1 :]
        return os.path.join(cmm_root, *relative_parts)
...
def build_messages(
    ...
    audio_path = resolve_media_path(sample["audio_path"], data_file, cmm_root)
    ...
    return [{"role": "user", "content": frames + [audio, prompt]}]
...
def run(args: argparse.Namespace) -> None:
    ...
    for sample in tqdm(samples, desc="CMM audio-driven"):
    ...
    json_path = os.path.join(
        args.output_dir, f"{model_name}-CMM-audio-driven-results.json"
    )
    hidden_path = os.path.join(
        args.output_dir, f"{model_name}-CMM-audio-driven-hiddenstates.pt"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data_file", default="cmm-audio-driven.json"
    )
    parser.add_argument(
        "--model_path",
