...
def build_messages(sample, args):
    audio_path = os.path.join(args.media_root, sample["audio_path"])
    image_path = os.path.join(args.media_root, sample["image_path"])
    opt_lines = "\n".join(
        f"{OPTION_LABELS[i]}. {o}" for i, o in enumerate(sample["options"]))
    prompt = (
        f"Please answer the following question based on the given image and audio:\n"
        f"{sample['question']}.\n"
        f"Please choose only one answer from the following options:\n"
        f"{opt_lines}."
    )
    return [{
        "role": "user",
...
def run(args):
            ...
            with torch.no_grad():
                gen = model.generate(
                    **inputs, use_audio_in_video=USE_AUDIO_IN_VIDEO,
                    return_audio=False, thinker_max_new_tokens=args.max_new_tokens, thinker_do_sample=False)
    ...
    coeff_tag = f"{coeff:g}"
    out_json = os.path.join(args.output_dir, f"{model_name}-{distractor_tag}-{DS}-({coeff_tag}).json")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--data_file",
                    default='omnibench_index.json')
    ap.add_argument("--media_root",
                    default='extract_data',
                    help="root directory for audio_path/image_path (should contain mm_data/ beneath it)")
    ...
    ap.add_argument("--distractor", choices=["text", "vision", "audio"], nargs="+",
                default=["vision"], help="distractor modality; default: vision")
    ...
    ap.add_argument("--output_dir", default='omnibench',
